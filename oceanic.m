%OCEANIC
% This routine defines the set of parameters relevant for an oceanic
% crust, with fault zone properties corresponding to a "granite" fault
...
%bottom melting temperature
%source: Jaupart and Mareschal 2007
Tm = 1350;
%rock average thermal conductivity in W/K/m
%source: Parsons and Scalter JGR 1977
kT = 3.14;
%Age of oceanic crust (in seconds !)
age = 1.25e6*365*24*3600;
...
%normal stress in Pa
sn = rho_rock*g*depth + 20e6;

%pore pressure in Pa
p0 = 1e3*g*depth + 20e6;

%temperature in C (from a cooling plate model)
T0 = 0 + Tm *erf(depth./(2*sqrt((kT/rhoc)*age)));